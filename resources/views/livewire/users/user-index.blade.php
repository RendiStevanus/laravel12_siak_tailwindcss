    <div
        ...
        <div class="flex flex-col gap-4 px-6 mb-2 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                ...
                <x-buttons.button-add href="/users/create" title="Add User" />
            </div>
            <div class="flex items-center gap-3">
                <form>
                    <div class="relative">
                        ...
                        <tr class="border-b border-gray-100 dark:border-white/[0.05]">
                            ...
                            <td class="px-4 sm:px-6 py-3.5">
                                @forelse ($user->roles as $role)
                                    <span class="px-2 py-1 rounded text-sm {{ $user->badgeColor($role->name) }}">
                                        {{ $role->name }}
                                    </span>
                                @empty
                            ...
                            <td class="px-4 sm:px-6 py-3.5">
                                <p class="text-gray-700 text-theme-sm dark:text-gray-400">
                                    {{ $user->program_studi?->nama ?? '-' }}</p>
                            </td>

                            <td class="px-4 sm:px-6 py-3.5 flex gap-1">
                                <x-buttons.button-edit href="/users/{{ $user->id }}/edit" title="Add User" />
